                                ...
                                <div class="row">
                                    <div class="row">
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <textarea
                                                    name="title"
                                                    id="title"
                                                    class="form-control char-textarea @error('title') is-invalid @else {{ old('title', $blog->title_en ?? '') ? 'is-valid' : '' }} @enderror"
                                                    placeholder="Title"
                                                    required
                                                >{{ old('title', $blog->title_en ?? '') }}</textarea>
                                                @error('title')
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <textarea
                                                    name="title_ar"
                                                    id="title_ar"
                                                    class="form-control char-textarea @error('title_ar') is-invalid @else {{ old('title_ar', $blog->title_ar ?? '') ? 'is-valid' : '' }} @enderror"
                                                    placeholder="العنوان"
                                                    required
                                                >{{ old('title_ar', $blog->title_ar ?? '') }}</textarea>
                                                @error('title_ar')
                                                <span class="col-form-label-sm text-danger">{{ $message }}</span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <textarea
                                                    name="desc"
                                                    id="desc"
                                                    class="form-control char-textarea @error('desc') is-invalid @else {{ old('desc', $blog->desc_en ?? '') ? 'is-valid' : '' }} @enderror"
                                                    placeholder="Description.."
                                                >{{ old('desc', $blog->desc_en ?? '') }}</textarea>
                                                @error('desc')
                                                <span class="col-form-label-sm text-danger">{{ $message }}</span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <textarea
                                                    name="desc_ar"
                                                    id="desc_ar"
                                                    class="form-control char-textarea @error('desc_ar') is-invalid @else {{ old('desc_ar', $blog->desc_ar ?? '') ? 'is-valid' : '' }} @enderror"
                                                    placeholder="{{ __('general.Description in arabic') }}.."
                                                >{{ old('desc_ar', $blog->desc_ar ?? '') }}</textarea>
                                                @error('desc_ar')
                                                <span class="col-form-label-sm text-danger">{{ $message }}</span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <input
                                                    value="{{ old('by', $blog->by ?? '') }}"
                                                    name="by"
                                                    type="text"
                                                    id="by"
                                                    class="form-control form-control-sm @error('by') is-invalid @else {{ old('by', $blog->by ?? '') ? 'is-valid' : '' }} @enderror"
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group">
                                                ...
                                                <input
                                                    ...
                                                    class="form-control flatpickr-human-friendly form-control-sm @error('date') is-invalid @else {{ old('date', $blog->date ?? '') ? 'is-valid' : '' }} @enderror"
                                    ...
                                    <div class="col-12 mt-4 mb-2">
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <select name="tags[]" multiple class="form-control testselect2 mb-1" id="tags">
                                                        @foreach($tags as $tag)
                                                            <option value="{{ $tag->id }}"
                                                                {{ (collect(old('tags', $blog->tags->pluck('id')->toArray()))->contains($tag->id)) ? 'selected' : '' }}>
                                                                {{ App::getLocale() == 'ar' ? $tag->title_ar : $tag->title_en }}
                                                            </option>
                                                        @endforeach
                                                        <option value="add_new_tag" data-class="add-new-tag-option">
                                                            + {{ __('general.Add New Tag') }}
                                                        </option>
                                                    </select>
                                                    @error('tags')
                                                    <span class="col-form-label-sm text-danger">{{ $message }}</span>
                                        ...
                                        <div id="blog-details-container">
                                            @php
                                                $existingDetails = $blog->details->toArray();
                                                $oldDetails = old('details');
                                                $details = [];

                                                if (is_array($oldDetails)) {
                                                    $details = $oldDetails;
                                                    $existingRemaining = array_slice($existingDetails, count($oldDetails));
                                                    foreach ($existingRemaining as $item) {
                                                        $details[] = $item;
                                                    }
                                                } else {
...
@section('js')
    ...
    {{--    add blog details   --}}
    <script>
        if (typeof blogDetailIndex === 'undefined') {
            var blogDetailIndex = 0;
        }

        function addBlogDetailRow() {
            const index = blogDetailIndex;
            ...
            <div class="blog-detail-box position-relative show mb-2" data-index="${index}">
                <div class="row">
                    <div class="col-md-4 mb-2">
                        <label class="form-label-sm text-muted">{{ __('general.URL') }}</label>
                        <input type="url" name="details[${index}][url]" class="form-control form-control-sm" placeholder="https://example.com">
                    </div>
                    <div class="col-md-4 mb-2">
                        <label class="form-label-sm text-muted">{{ __('general.Key (English)') }}</label>
                        <input type="text" name="details[${index}][key_url_en]" class="form-control form-control-sm" placeholder="english-key">
                    </div>
                    <div class="col-md-4 mb-2">
                        <label class="form-label-sm text-muted">{{ __('general.Key (Arabic)') }}</label>
                        <input type="text" name="details[${index}][key_url_ar]" class="form-control form-control-sm" placeholder="arabic-key">
                    </div>
                </div>
        ...
        `);
            ...
            setTimeout(() => row.addClass('show'), 100);
