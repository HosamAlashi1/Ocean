    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($member) {
                return view('components.datatable.actions', ['routeEdit'=>'member.edit','routeDelete'=>'member.destroy','id' => $member->id,'name' => $member->name_en]);
            })
            ->addColumn('image', function ($member) {
                return view('components.datatable.image', ['photo' => $member->image]);
            })
            ->addColumn('created_at', function ($member) {
                return $member->created_at->format('Y-m-d H:i');
            })
            ->rawColumns(['action'])
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('name_' . (App::getLocale() === 'ar' ? 'ar' : 'en'))
                ...
                ->addClass('text-center align-middle'),
            Column::make('job_name_' . (App::getLocale() === 'ar' ? 'ar' : 'en'))
                ->title(__('dataTable.job_name')) // Translate the title
                ->addClass('text-center align-middle'),
